...
*************************************************************************/
{
  ...
  Nutshell.start = (el = document.body) => {
    ...
    if (window == window.top) {
      ...
      Nutshell.htmlCache[Nutshell.thisPageURL] = _purifyHTML(
        el.innerHTML,
        Nutshell.thisPageURL
      );
  ...
  Nutshell.convertLinksToExpandables = (dom, forThisElement) => {
    ...
    expandables.forEach((ex) => {
      ...
      let linkStyle = window.getComputedStyle(
        forThisElement ? forThisElement : ex
      );
      ...
      if (ex.nextSibling && ex.nextSibling.nodeValue) {
        ...
        while (
          keepPunctuation.indexOf((nextChar = ex.nextSibling.nodeValue[0])) >= 0
        ) {
      ...
      if (hasWordsAfterExpandable) {
        ...
        let textBeforeThinkLink = tmpSpan.innerText,
          sentencesBeforeThisLink = textBeforeThinkLink.split(
            Nutshell.getLocalizedText("endPunctuation")
          ),
          lastSentenceHTML =
            sentencesBeforeThisLink[sentencesBeforeThisLink.length - 1];
      ...
      ex.updateFollowupText = () => {
        if (!bubble || !hasWordsAfterExpandable) {
          ...
          followupSpan.innerHTML = longEnough
            ? longFollowupHTML
            : shortFollowupHTML;
        }
      };
      ...
      ex.open = (mouseEvent) => {
        ...
        //debugger;
        let clickX =
          mouseEvent.clientX - ex.parentNode.getBoundingClientRect().x; // relative to parent, I guess???
    ...
    let allExpandables = [...document.querySelectorAll(".nutshell-expandable")],
      nestedExpandables = [
        ...document.querySelectorAll(
          ".nutshell-expandable .nutshell-expandable"
        ),
      ];
        ...
        fetch(resourceURL)
          ...
          .then((data) => {
            ...
            if (data.query.pages[pageKey].thumbnail) {
              pageHTML =
                `<img width=300 src='${data.query.pages[pageKey].thumbnail.source}' data-float=right />` +
                pageHTML;
            }
          ...
          fetch(url)
            ...
            .catch((err) => {
              ...
              if (err.message == "404") {
                ...
                // Set up listener...
                let _messageListener = window.addEventListener(
                  "message",
                  (message) => {
        ...
        .then((purifiedHTML) => {
          if (_isWikipedia(url) || _isYouTube(url)) {
            ...
            if (isSearchingByHeading) {
              ...
              if (!foundHeading) {
                containerHTML = `<p>${Nutshell.getLocalizedText(
                  "sectionIDError"
                ).replace("[ID]", sectionID)}</p>`;
              } else {
                ...
                while (!foundEndOfSection) {
                  ...
                  if (currentNode) {
                    ...
                    if (currentNode.tagName) {
                      ...
                      if (
                        HEADER_TAGS.indexOf(currentTag) >= 0 ||
                        currentTag == "hr"
                      ) {
              ...
              if (queryKeys.start) {
                ...
                if (!found) {
                  ...
                  if (queryKeys.length || queryKeys.end) {
                    ...
                    let elementsLeft = queryKeys.length
                      ? parseInt(queryKeys.length) - 1
                      : Infinity;
            ...
            if (queryKeys.before) {
              containerHTML =
                _decodeParsePurifyItalics(queryKeys.before) + containerHTML;
            }
            if (queryKeys.after) {
              containerHTML =
                containerHTML + _decodeParsePurifyItalics(queryKeys.after);
            }
          }
  ...
  let _decodeParsePurifyItalics = (whatever) => {
    return (
      "<i>" +
      DOMPurify.sanitize(marked.parse(decodeURIComponent(whatever))) +
      "</i>"
    );
  };
  ...
  Nutshell.createBubble = (expandable, clickX) => {
    ...
    bubble.close = () => {
      ...
      Nutshell._nutshellsOpen -=
        bubble.querySelectorAll(".nutshell-bubble").length;
      Nutshell._updateCloseAllNutshells();
    };
  ...
  Nutshell.fillEmbedModalText = () => {
    ...
    _p1.innerHTML = Nutshell.getLocalizedText("embedStep1")
      .replace(
        `[HEAD]`,
        `<span style="font-family:monospace">&lt;head&gt;</span>`
      )
      .replace(
        `[CODE]`,
        `<input style="width:100%" value="<script src='${Nutshell.cdn}'></script>" onclick="select()"/>`
      );

    // Step 2: Link
    _p2.innerHTML = Nutshell.getLocalizedText("embedStep2").replace(
      `[LINK]`,
      `
    ...
    // (Learn More)
    _p4.innerHTML =
      `<a href='https://ncase.me/nutshell/' target='_blank'>` +
  ...
  Nutshell.showEmbedModal = (url, linkText) => {
    ...
    _p0.innerHTML = Nutshell.getLocalizedText("embedStep0").replace(
      `[EXAMPLE]`,
      `<a href='${url}' style='font-weight:bold'>:${linkText}</a>`
    );
    ...
    return (function t() {
      ...
      var de,
        ...
        it = function (t) {
          (rt && rt === t) ||
            ...
            (xe = "ALLOWED_ATTR" in t ? O({}, t.ALLOWED_ATTR) : Se),
            (Ve =
              "ADD_URI_SAFE_ATTR" in t ? O(D($e), t.ADD_URI_SAFE_ATTR) : $e),
            (Ye =
              "ADD_DATA_URI_TAGS" in t ? O(D(Ke), t.ADD_DATA_URI_TAGS) : Ke),
            ...
            t.ADD_ATTR && (xe === Se && (xe = D(xe)), O(xe, t.ADD_ATTR)),
      ...
      var mt = function (e) {
        ...
        vt = function (e) {
          ...
          var n = he(e.nodeName);
          if (
            (bt("uponSanitizeElement", e, { tagName: n, allowedTags: Ae }),
          ...
          if (!Ae[n] || _e[n]) {
            if (!_e[n] && Nt(n)) {
              if (ke.tagNameCheck instanceof RegExp && S(ke.tagNameCheck, n))
                return !1;
              if (ke.tagNameCheck instanceof Function && ke.tagNameCheck(n))
                return !1;
            }
            if (je && !We[n]) {
          ...
          if (i) {
            ...
            for (a = i.length; a--; ) {
              ...
              if (
                ...
                (n = l.attrValue),
                !l.forceKeepAttr && (pt(u, e), l.keepAttr))
              )
                if (S(/\/>/i, n)) pt(u, e);
                else {
        ...
        At = function e(t) {
          ...
          for (bt("beforeSanitizeShadowDOM", t, null); (n = r.nextNode()); )
            bt("uponSanitizeShadowNode", n, null),
              vt(n) || (n.content instanceof l && e(n.content), Et(n));
      ...
      return (
        (r.sanitize = function (t, o) {
          var i, c, s, m, f;
          if (
            ((et = !t) && (t = "\x3c!--\x3e"), "string" != typeof t && !yt(t))
          ) {
          ...
          if (
            ...
            (1 ===
              (c = (i = dt("\x3c!----\x3e")).ownerDocument.importNode(t, !0))
                .nodeType &&
              "BODY" === c.nodeName) ||
            "HTML" === c.nodeName
              ? (i = c)
          ...
          for (var d = ht(Pe ? t : i); (s = d.nextNode()); )
            (3 === s.nodeType && s === m) ||
              vt(s) ||
              (s.content instanceof l && At(s.content), Et(s), (m = s));
            ...
            if (t) {
              ...
              for (var E = f.items.length, x = 0; x < E; x++) {
                (this.lexer.state.top = !1),
                  (f.items[x].tokens = this.lexer.blockTokens(
                    f.items[x].text,
                    []
                  ));
              ...
              if (t.header.length === t.align.length) {
                ...
                for (i = t.rows.length, u = 0; u < i; u++)
                  for (r = t.rows[u], n = 0; n < r.length; n++)
                    (r[n].tokens = []),
                      this.lexer.inline(r[n].text, r[n].tokens);
                return t;
              }
            }
          }),
    ...
    var z = (function () {
        ...
        return (
          ...
          (n.inlineTokens = function (r, t) {
            ...
            if (this.tokens.links) {
              ...
              if (0 < D.length)
                for (
                  ;
                  null !=
                  (n = this.tokenizer.rules.inline.reflinkSearch.exec(o));
            ...
            for (
              ;
              null != (n = this.tokenizer.rules.inline.blockSkip.exec(o));
    ...
    function I(e, u, n) {
      ...
      if (
        ...
        var s = function (t) {
          ...
          if (!t)
            try {
              u.walkTokens && I.walkTokens(r, u.walkTokens),
                (e = R.parse(r, u));
            } catch (e) {
              t = e;
            }
